...
function pn_node($node, $mode = 'n') {
  ...
  if ($n_nid) {
    ...
    if ($n_node->type=='section_page' || $n_node->type=='event'){ // article and event are the machine name of a content type
      drupal_add_html_head_link(array(
         ...
         // output by Drupal.
         'href' => url('node/' . $n_node->nid, array('absolute' => TRUE)),
     ));
        $html = l('"' . $n_node->title . '"', 'node/' . $n_node->nid, array('attributes' => array('rel' => array($link_text),'class' => $link_text, 'title' => $n_node->title)));
        return $html;
    }
